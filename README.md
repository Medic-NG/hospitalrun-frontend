...
Frontend is the primary repository and is used for development purposes. Server is the node backend, if you are looking to run medihos in a clinical setting, you should use server.
...
- [Accessing medihos with Docker Toolbox](#accessing-medihos-with-docker-toolbox)
- [Accessing medihos with Docker or Docker-compose](#accessing-medihos-with-docker-or-docker-compose)
...
5. Clone this repo with `git clone https://github.com/Dewdrops-NG/medihos-frontend`, go to the cloned folder and run 

```
yarn install
```

  - **Note:** *If you just want to use the project, cloning is the best option. However, if you wish to contribute to the project, you will need to fork the project first, and then clone your `medihos-frontend` fork and make your contributions via a branch on your fork.*
6. Install and configure [CouchDB](http://couchdb.apache.org/):
    ...
    4. Create admin user:
        1. If you are running CouchDB 1.x:
            1. If you have just installed CouchDB and have no admin user, please run `./script/initcouch.sh` in the folder you cloned the medihos repo.  A user `hradmin` will be created with password `test`.
            2. If you already have a CouchDB admin user, please run `./script/initcouch.sh USER PASS` in the folder you cloned the medihos repo.  `USER` and `PASS` are the CouchDB admin user credentials.
        2. If you are running CouchDB 2.x (experimental):
            1. medihos currently does not fully support CouchDB 2.x, but you are welcome to try using it.  Most functionality should work but currently creating and/or editing users does not work in CouchDB 2.x.  See https://github.com/Dewdrops-NG/medihos-frontend/issues/953 for more details.
            2. If you have just installed CouchDB and have no admin user, please run `./script/initcouch2.sh` in the folder you cloned the medihos repo.  A user `hradmin` will be created with password `test`.
            3. If you already have a CouchDB admin user, please run `./script/initcouch2.sh USER PASS` in the folder you cloned the medihos repo.  `USER` and `PASS` are the CouchDB admin user credentials.
7. Copy the `server/config-example.js` to `server/config.js` in the folder you cloned the medihos repo.  If you already had a CouchDB admin user that you passed into the couch script (`./script/initcouch.sh USER PASS`), then you will need to modify the `couchAdminUser` and `couchAdminPassword` values in `server/config.js` to reflect those credentials. (*Note: If on Mac, you need to make sure CouchDB can be run. See [How to open an app from an unidentified developer and exempt it from Gatekeeper](https://support.apple.com/en-us/HT202491).*)
...
To run medihos with [Docker](https://www.docker.com/) please do the following:
- Go to [https://docs.docker.com/engine/installation](https://docs.docker.com/engine/installation) to download and install Docker.
- Clone the repository with the command `git clone https://github.com/Dewdrops-NG/medihos-frontend.git`.
- Change to the medihos-frontend directory `cd medihos-frontend`.
- Build the medihos image with `docker build -t medihos-frontend .`.
- Execute `docker run -it --name couchdb -d couchdb:1.7.1` to create the couchdb container.
- Execute `docker run -it --name medihos-frontend -p 4200:4200 --link couchdb:couchdb -d medihos-frontend` to create the medihos container.

### Running with Docker Compose
To run medihos with Docker-compose please do the following:
...
### Accessing medihos with Docker Toolbox
If you are running with Docker Toolbox you will have to run the following commands to get the IP of the docker machine where medihos-frontend is running with the following:
...
### Accessing medihos with Docker or Docker-compose
...
The app will usually tell you when something needs to happen (i.e. if you try to `yarn start` and npm is out of date, it will tell you to run `yarn update`.
